define(function() {
  ...
  var util = {
    ...
    typeOf: function(object, type) {
      ...
      if (result == 'object') {
        result = toString.call(object).slice(8, -1).toLowerCase(); // Array, Date, RegExp, null
      }
     ...
     *
     *     util.cast(2, 'string'); //=> '2'
     *     util.cast([1, 2], 'string'); //=> '1,2'
     *     util.cast('one,two,three', 'array'); //=> ['one', 'two', 'three']
     *
    ...
    cast: function(value, type) {
      ...
      if (type == 'number')  return valType == 'string' ? parseFloat(value) : Number(value);
     ...
     * @param {Object}  target  Target object
     * @param {Object}  source  Source object
     * @param {Boolean} mutable If set to true, target object will be changed
     */
